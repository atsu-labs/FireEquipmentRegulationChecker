...
import { ref } from 'vue';
import { useArticle21Logic, type Article21UserInput } from '../composables/articles/article21Logic';
import type { Floor } from '../types';

describe('useArticle21Logic', () => {
  const createMockUserInput = (overrides: Partial<Article21UserInput>): Article21UserInput => {
  ...
  it('1号ロ: (六)項ハの宿泊施設で設置が必要になる', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item06_ha_1'), hasLodging: ref(true) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('2号: (九)項イで延べ面積200㎡以上の場合', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item09_i'), totalArea: ref(200) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('3号イ: (四)項で延べ面積300㎡以上の場合', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item04'), totalArea: ref(300) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('3号ロ: (六)項ハの宿泊施設なしで延べ面積300㎡以上の場合', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item06_ha_2'), hasLodging: ref(false), totalArea: ref(300) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('4号: (七)項で延べ面積500㎡以上の場合', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item07'), totalArea: ref(500) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
    ...
    const userInput = createMockUserInput({ buildingUse: ref('item16_3'), totalArea: ref(500) });
    const { regulationResult } = useArticle21Logic(userInput);
  ...
  it('6号: (十一)項で延べ面積1000㎡以上の場合', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item11'), totalArea: ref(1000) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('7号: 対象用途で特定一階段の場合', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item02_i'), isSpecifiedOneStaircase: ref(true) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('7号: 対象用途だが、特定一階段でない場合', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item02_i'), isSpecifiedOneStaircase: ref(false) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(false);
  });

  it('7号: 対象用途でない場合', () => {
    const userInput = createMockUserInput({ buildingUse: ref('item07'), isSpecifiedOneStaircase: ref(true) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(false);
  });

  it('8号: 指定可燃物を基準数量の500倍以上貯蔵・取り扱いしている場合', () => {
    const userInput = createMockUserInput({ storesCombustiblesOver500x: ref(true) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('8号: 指定可燃物を基準数量の500倍以上貯蔵・取り扱いしていない場合', () => {
    const userInput = createMockUserInput({ storesCombustiblesOver500x: ref(false) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(false);
  });
  ...
  it('10号: (2)項イで地階の面積が100㎡以上', () => {
    ...
    const userInput = createMockUserInput({ buildingUse: ref('item02_i'), floors });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('10号: (3)項イで無窓階の面積が100㎡以上', () => {
    ...
    const userInput = createMockUserInput({ buildingUse: ref('item03_i'), floors });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
    expect(regulationResult.value.basis).toBe('令第21条第1項第10号');
  });
  ...
  it('12号: 道路の用に供される部分（屋上）が600㎡以上の場合', () => {
    const userInput = createMockUserInput({ hasRoadPart: ref(true), roadPartRooftopArea: ref(600) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('12号: 道路の用に供される部分（屋上以外）が400㎡以上の場合', () => {
    const userInput = createMockUserInput({ hasRoadPart: ref(true), roadPartOtherArea: ref(400) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
    expect(regulationResult.value.basis).toBe('令第21条第1項第12号');
  });
  ...
  it('13号: 駐車の用に供する部分が200㎡以上で、同時に屋外に出られない場合', () => {
    const userInput = createMockUserInput({ hasParkingPart: ref(true), parkingPartArea: ref(200), canAllVehiclesExitSimultaneously: ref(false) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('15号: 通信機器室が500㎡以上の場合', () => {
    const userInput = createMockUserInput({ hasTelecomRoomOver500sqm: ref(true) });
    const { regulationResult } = useArticle21Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
