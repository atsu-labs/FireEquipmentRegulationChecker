...
describe("useArticle22Logic", () => {
  const createMockUserInput = (
    overrides: Partial<Article22UserInput>
  ): Article22UserInput => {
  ...
  it("特殊な構造でない場合は設置不要", () => {
    const userInput = createMockUserInput({
      hasSpecialCombustibleStructure: ref(false),
    });
  ...
  it("6号: (16)項イで特定用途部分の面積合計が300㎡以上の場合に設置義務あり", () => {
    const userInput = createMockUserInput({
      buildingUse: ref("annex16_i"),
      totalArea: ref(600),
      floors: ref([
        {
          ...
          isWindowless: false,
          componentUses: [
            { useCode: "annex04", floorArea: 200, capacity: null }, // 飲食店（特定用途）
            { useCode: "annex15", floorArea: 100, capacity: null }, // 事務所（非特定用途）
          ],
        },
        {
          level: 2,
          ...
          isWindowless: false,
          componentUses: [
            { useCode: "annex06_i_1", floorArea: 150, capacity: null }, // 病院（特定用途）
            { useCode: "annex15", floorArea: 150, capacity: null }, // 事務所（非特定用途）
          ],
        },
      ]),
    });
    ...
    expect(regulationResult.value.required).toBe(true);
    expect(regulationResult.value.basis).toBe("令第22条第1項第6号");
    expect(regulationResult.value.message).toContain("木造モルタル構造等で");
    expect(regulationResult.value.message).toContain("350㎡");
  });

  it("6号: (16)項イで特定用途部分の面積合計が300㎡未満の場合は設置義務なし", () => {
    const userInput = createMockUserInput({
      buildingUse: ref("annex16_i"),
      totalArea: ref(600),
      floors: ref([
        {
          ...
          isWindowless: false,
          componentUses: [
            { useCode: "annex04", floorArea: 100, capacity: null }, // 飲食店（特定用途）
            { useCode: "annex15", floorArea: 200, capacity: null }, // 事務所（非特定用途）
          ],
        },
        {
          level: 2,
          ...
          isWindowless: false,
          componentUses: [
            { useCode: "annex06_i_1", floorArea: 100, capacity: null }, // 病院（特定用途）
            { useCode: "annex15", floorArea: 200, capacity: null }, // 事務所（非特定用途）
          ],
        },
      ]),
    ...
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(false);
    expect(regulationResult.value.message).toContain("200㎡");
  ...
  it("7号: (1)項で契約電流容量が50Aを超える", () => {
    const userInput = createMockUserInput({
      ...
      contractedCurrentCapacity: ref(51),
    });
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it("7号: (1)項で契約電流容量が50Aちょうど", () => {
    const userInput = createMockUserInput({
      buildingUse: ref("annex01_i"),
      contractedCurrentCapacity: ref(50),
    });
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(false);
  });
