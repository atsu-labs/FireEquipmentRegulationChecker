...
describe('useArticle22Logic', () => {
  ...
  it('2号: (5)項で延べ面積150㎡以上', () => {
    const userInput = createMockUserInput({ buildingUse: ref('annex05_i'), totalArea: ref(150) });
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('3号: (6)項で延べ面積300㎡以上', () => {
    const userInput = createMockUserInput({ buildingUse: ref('annex06_i_1'), totalArea: ref(300) });
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('4号: (10)項で延べ面積500㎡以上', () => {
    const userInput = createMockUserInput({ buildingUse: ref('annex10'), totalArea: ref(500) });
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('5号: (14)項で延べ面積1000㎡以上', () => {
    const userInput = createMockUserInput({ buildingUse: ref('annex14'), totalArea: ref(1000) });
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('6号: (16)項イで延べ面積500㎡以上の場合にwarningを返す', () => {
    const userInput = createMockUserInput({ buildingUse: ref('annex16_i'), totalArea: ref(500) });
    const { regulationResult } = useArticle22Logic(userInput);
  ...
  it('7号: (1)項で契約電流容量が50Aを超える', () => {
    const userInput = createMockUserInput({ buildingUse: ref('annex01_i'), contractedCurrentCapacity: ref(51) });
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('7号: (1)項で契約電流容量が50Aちょうど', () => {
    const userInput = createMockUserInput({ buildingUse: ref('annex01_i'), contractedCurrentCapacity: ref(50) });
    const { regulationResult } = useArticle22Logic(userInput);
    expect(regulationResult.value.required).toBe(false);
  });
