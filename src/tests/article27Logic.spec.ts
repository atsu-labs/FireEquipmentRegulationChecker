...
import type { Floor, Article27UserInput } from '@/types';
...
describe('useArticle27Logic', () => {
  ...
  it('1号: 敷地面積20000㎡以上、その他建築物5000㎡以上で設置義務あり', () => {
    const floors: Floor[] = [{ level: 1, type: 'ground', floorArea: 5000, capacity: null, isWindowless: false, componentUses: [] }];
  ...
  it('1号: 準耐火建築物10000㎡以上で設置義務あり', () => {
    const floors: Floor[] = [{ level: 1, type: 'ground', floorArea: 10000, capacity: null, isWindowless: false, componentUses: [] }];
  ...
  it('1号: 耐火建築物15000㎡以上で設置義務あり', () => {
    const floors: Floor[] = [
        { level: 1, type: 'ground', floorArea: 10000, capacity: null, isWindowless: false, componentUses: [] },
        { level: 2, type: 'ground', floorArea: 5000, capacity: null, isWindowless: false, componentUses: [] },
    ];
  ...
  it('1号: 床面積が基準未満の場合、設置義務なし', () => {
    const floors: Floor[] = [{ level: 1, type: 'ground', floorArea: 4999, capacity: null, isWindowless: false, componentUses: [] }];
    const input = createMockInput({ siteArea: ref(20000), floors: ref(floors), buildingStructure: ref('other') });
    const { regulationResult } = useArticle27Logic(input);
    expect(regulationResult.value.required).toBe(false);
  });
