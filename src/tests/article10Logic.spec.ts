...
describe('Article10Logic', () => {
  ...
  const userInput = createMockUserInput({ buildingUse: ref('annex01_i') });
  const { regulationResult } = useArticle10Logic(userInput);
  ...
  const userInput = createMockUserInput({ buildingUse: ref('annex03_i'), usesFireEquipment: ref(true) });
  const { regulationResult } = useArticle10Logic(userInput);
  expect(regulationResult.value.required).toBe(true);
  ...
  const userInput = createMockUserInput({ buildingUse: ref('annex04'), totalFloorAreaInput: ref(150) });
  const { regulationResult } = useArticle10Logic(userInput);
      expect(regulationResult.value.required).toBe(true);
    ...
    it('イ: (4)項で延べ面積が150㎡未満の場合、(二号の事由では)設置義務なしと判定されること', () => {
      const userInput = createMockUserInput({ buildingUse: ref('annex04'), totalFloorAreaInput: ref(149) });
      const { regulationResult } = useArticle10Logic(userInput);
      // 他の条件に該当しない限りfalse
      expect(regulationResult.value.required).toBe(false);
    });
    ...
    it('ロ: (3)項で火気設備がなく、延べ面積が150㎡以上の場合、設置義務ありと判定されること', () => {
      const userInput = createMockUserInput({ buildingUse: ref('annex03_i'), totalFloorAreaInput: ref(150), usesFireEquipment: ref(false) });
      const { regulationResult } = useArticle10Logic(userInput);
      expect(regulationResult.value.required).toBe(true);
  ...
  describe('令第十条第一項三号', () => {
    it('(7)項で延べ面積が300㎡以上の場合、設置義務ありと判定されること', () => {
      const userInput = createMockUserInput({ buildingUse: ref('annex07'), totalFloorAreaInput: ref(300) });
      const { regulationResult } = useArticle10Logic(userInput);
      expect(regulationResult.value.required).toBe(true);
    ...
    it('(7)項で延べ面積が300㎡未満の場合、(三号の事由では)設置義務なしと判定されること', () => {
      const userInput = createMockUserInput({ buildingUse: ref('annex07'), totalFloorAreaInput: ref(299) });
      const { regulationResult } = useArticle10Logic(userInput);
      expect(regulationResult.value.required).toBe(false);
    });
  });
  ...
  describe('令第十条第一項四号', () => {
    it('少量危険物を貯蔵している場合、設置義務ありと判定されること', () => {
      const userInput = createMockUserInput({ buildingUse: ref('annex16'), storesMinorHazardousMaterials: ref(true) });
      const { regulationResult } = useArticle10Logic(userInput);
      expect(regulationResult.value.required).toBe(true);
    ...
    it('指定可燃物を貯蔵している場合、設置義務ありと判定されること', () => {
      const userInput = createMockUserInput({ buildingUse: ref('annex16'), storesDesignatedCombustibles: ref(true) });
      const { regulationResult } = useArticle10Logic(userInput);
      expect(regulationResult.value.required).toBe(true);
      expect(regulationResult.value.basis).toBe('令第十条第一項四号');
    });
