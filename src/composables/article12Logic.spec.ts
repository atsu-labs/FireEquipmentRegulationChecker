...
const createMockInput = (overrides: Partial<Article12UserInput> = {}): Article12UserInput => {
  ...
  // Create a new object for the merged result to avoid modifying defaults
...
describe('useArticle12Logic', () => {
  ...
  describe('令第12条第1項第1号（避難困難施設等）', () => {
    it('1号ロ: 用途が(6)項ロ(1)で延焼抑制構造でない場合、設置義務あり', () => {
      const input = createMockInput({ buildingUse: ref('item06_ro_1'), hasFireSuppressingStructure: ref(false) });
      const { regulationResult } = useArticle12Logic(input);
    ...
    it('1号ハ: 用途が(6)項ロ(2)で介助が必要な者が入所し、面積275㎡未満でも延焼抑制構造でない場合、設置義務あり', () => {
      ...
      expect(regulationResult.value.message).toContain('介助がなければ避難できない者を主として入所させる施設');
    });
    ...
    it('1号ハ: 用途が(6)項ロ(4)で介助が必要な者以外で、面積275㎡以上で延焼抑制構造でない場合、設置義務あり', () => {
      ...
      expect(regulationResult.value.required).toBe(true);
      expect(regulationResult.value.basis).toBe('令第12条第1項第1号ハ');
      expect(regulationResult.value.message).toContain('介助がなければ避難できない者を主として入所させるもの以外');
    });
    ...
    it('1号: 延焼抑制構造である場合、設置義務なし', () => {
      const input = createMockInput({ buildingUse: ref('item06_ro_1'), hasFireSuppressingStructure: ref(true) });
      const { regulationResult } = useArticle12Logic(input);
      expect(regulationResult.value.required).toBe(false);
    });
  });
