...
// テストデータ生成のヘルパー関数
const createMockInput = (overrides: Partial<Article11UserInput> = {}): Article11UserInput => {
  const defaults: Article11UserInput = {
    ...
    isFlammableItemsAmountOver750: ref(false),
  };
...
describe('useArticle11Logic', () => {
  ...
  it('用途がitem01で延べ面積500㎡以上なら設置義務あり', () => {
    const input = createMockInput({ buildingUse: ref('item01'), totalArea: ref(600) });
    const { regulationResult } = useArticle11Logic(input);
  ...
  it('用途がitem02で延べ面積700㎡未満なら設置義務なし', () => {
    const input = createMockInput({ buildingUse: ref('item02'), totalArea: ref(500) });
    const { regulationResult } = useArticle11Logic(input);
    expect(regulationResult.value.required).toBe(false);
  });

  it('用途がitem11で延べ面積1000㎡以上なら設置義務あり', () => {
    const input = createMockInput({ buildingUse: ref('item11'), totalArea: ref(1200) });
    const { regulationResult } = useArticle11Logic(input);
    expect(regulationResult.value.required).toBe(true);
  ...
  it('指定可燃物を750倍以上貯蔵している場合、設置義務あり', () => {
    const input = createMockInput({ buildingUse: ref('item01'), storesFlammableItems: ref(true), isFlammableItemsAmountOver750: ref(true) });
    const { regulationResult } = useArticle11Logic(input);
    expect(regulationResult.value.required).toBe(true);
