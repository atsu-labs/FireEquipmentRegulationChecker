...
export function useArticle11Logic(userInput: Article11UserInput) {
  const regulationResult = computed((): JudgementResult => {
    ...
    let totalAreaRuleInfo: { requiredArea: number; basis: string } | null =
      null;
    ...
    if (useCodeMatches(buildingUse, ["item01"])) {
      ...
      if (currentTotalArea >= requiredArea) {
        ...
        return {
          required: true,
          message: `用途（${useDisplay}）が（1）項に該当し、延べ面積が${currentTotalArea.toFixed(
            2
    ...
    // 三号: (11)項, (15)項 → 延べ面積 1000㎡以上
      ...
      if (currentTotalArea >= requiredArea) {
        ...
        return {
          required: true,
          message: `用途（${useDisplay}）が（11）項または（15）項に該当し、延べ面積が${currentTotalArea.toFixed(
            2
          )}㎡（≧ ${requiredArea}㎡）のため、設置が必要です。${multiplierDescription}`,
          basis: `令第11条第1項第3号`,
        };
    ...
    // 四号: (16の2)項 → 延べ面積 150㎡以上
      ...
      if (currentTotalArea >= requiredArea) {
        ...
        return {
          required: true,
          message: `用途（${useDisplay}）が（16の2）項に該当し、延べ面積が${currentTotalArea.toFixed(
            2
          )}㎡（≧ ${requiredArea}㎡）のため、設置が必要です。${multiplierDescription}`,
          basis: `令第11条第1項第4号`,
        };
    ...
    // 五号: 指定可燃物を750倍以上
    if (
      userInput.storesFlammableItems.value &&
    ...
    if (!isAlreadyApplicable) {
      ...
      if (requiredFloorArea > 0) {
        ...
        if (userInput.hasBasement.value && basementArea >= requiredFloorArea) {
          return {
            required: true,
            message: `地階の床面積が${basementArea.toFixed(
              2
            )}㎡（≧ ${requiredFloorArea}㎡）のため、設置が必要です。${multiplierDescription}`,
            basis: "令第11条第1項第6号",
          };
        }
        if (
          userInput.hasNoWindowFloor.value &&
          noWindowFloorArea >= requiredFloorArea
        ) {
          return {
            required: true,
            message: `無窓階の床面積が${noWindowFloorArea.toFixed(
              2
            )}㎡（≧ ${requiredFloorArea}㎡）のため、設置が必要です。${multiplierDescription}`,
            basis: "令第11条第1項第6号",
          };
        ...
        if (
          userInput.hasUpperFloors.value &&
          upperFloorsArea >= requiredFloorArea
        ) {
          return {
            required: true,
            message: `4階以上の階の床面積が${upperFloorsArea.toFixed(
              2
            )}㎡（≧ ${requiredFloorArea}㎡）のため、設置が必要です。${multiplierDescription}`,
            basis: "令第11条第1項第6号",
          };
    ...
    if (totalAreaRuleInfo) {
      return {
        required: false,
        message: `設置義務はありません。延べ面積 ${currentTotalArea.toFixed(
          2
        )}㎡ は、規制対象の ${totalAreaRuleInfo.requiredArea.toFixed(
          2
        )}㎡ に達していません。${multiplierDescription}`,
        basis: "-",
      };
    }
